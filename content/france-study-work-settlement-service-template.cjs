...
<section id="route"><h2 class="zh-herald-section-head" data-num="01">${zh?'四段服務路線':'Four-stage service route'}</h2><div class="service-situation-grid">${stages.map(([n,t,d])=>`<article><b>${n}</b><strong>${t}</strong><p>${d}</p></article>`).join('')}</div></section>
<section id="process"><h2 class="zh-herald-section-head" data-num="02">${zh?'怎樣推進':'How the work progresses'}</h2><ol class="service-route-list">${process.map(([t,d])=>`<li><strong>${t}</strong><span>${d}</span></li>`).join('')}</ol></section>
