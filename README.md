...
每篇導報須遵守 [題圖與 X 分享發布流程](docs/HERALD-PUBLISHING-WORKFLOW.md)，並執行 `scripts/check-herald-share.py` 本地及線上檢查。必須實測指定平台草稿卡片，不能只憑部署成功或 HTTP 200 宣稱分享驗收通過。

潛客跟進固定採用「文章＋對應服務頁＋各頁專屬題圖＋原私訊發送」流程。服務頁須能說清服務與費用並可聯絡；兩頁互鏈、分別驗收分享預覽，再發給已交辦的潛客。詳見上述流程第 7 節。
