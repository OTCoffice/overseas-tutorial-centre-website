...
本書不是讓學生從頭到尾讀完就算完成，而是要在 assignment 的不同階段反覆使用：
...
| Business area | Common assignment challenge | Writing support in this toolkit | 中文提醒 |
| --- | --- | --- | --- |
...
不同 business topic 看起來不同，但寫作底層邏輯相同。學生不是要背很多模板，而是要掌握一套流程：
...
本書不是按「知識點」排列，而是按 assignment workflow 排列：先讀題，再理解深度，再規劃證據，再寫段落，再處理 AI / integrity，最後提交檢查。這種順序更符合 BTEC Business assignment 的實際工作流程。
...
Distinction-level 的核心不是「寫更多」，而是「判斷更成熟」。高階答案會回答：
...
AI 最大風險不是「用了就錯」，而是學生失去 authorship。比較安全的用法是幫助理解、提問、檢查邏輯；危險用法是生成完整答案、編造資料、改寫到看不出學生自己寫作能力。
...
這些問題不是形式，而是保護學生自己的 academic integrity。
...
BTEC assignment 最危險的是 replacement。學生可能以為「我只是讓 AI 潤色」，但如果 AI 已經重寫了觀點、證據、判斷和段落結構，那就不只是潤色，而是替代寫作。
...
建議每個 assignment 都建立一個簡單 support log。這不是增加負擔，而是保護自己。尤其是使用 AI、補習、同學討論、老師 feedback 時，最好留下記錄：
...
老師 feedback 不是讓學生機械補字數。比如老師寫 "needs more analysis"，不是讓你多加幾句空話，而是要你解釋：
